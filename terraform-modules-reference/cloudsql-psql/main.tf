locals {
  ...
  database_flags = merge({
    ...
    var.logical_decoding == "on" ? { "cloudsql.logical_decoding" = "on" } : {},
  ...
  availability_type = (
    var.availability_type != null ? var.availability_type : (
      lookup(local.availability_types, var.environment)
    )
  )

  enable_point_in_time_recovery = (
    var.backup_enabled == true ? var.enable_point_in_time_recovery : false
  )
...
resource "google_sql_database_instance" "cloudsql" {
  ...
  settings {
    ...
    dynamic "backup_configuration" {
      for_each = !var.backup_enabled || local.is_replica ? [] : [true]
